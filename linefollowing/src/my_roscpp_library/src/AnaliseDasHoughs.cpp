...
AnaliseDasHoughs::AnaliseDasHoughs(ConfigXML *_config) {
	config = _config;
	colorFrame = Mat3b(Size(config->camera.FrameSize.width, config->camera.FrameSize.height), Vec3b(0,0,0));

	posicaoCarro = Point2d(config->camera.FrameSize.width / 2.0, config->camera.FrameSize.height);
	posicaoCarroIPM = config->ipm->applyHomography(Point2d(config->camera.FrameSize.width / 2.0, (double)config->roi.height));
	larguraPista = (config->camera._IPM.tr - config->camera._IPM.tl);
}
