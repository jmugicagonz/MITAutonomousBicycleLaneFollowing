...
void Kalman::resetaKalman(KalmanFilter *KF, int m, int n) {
	// vetor de estado ter� 6 params
	// vetor de medida ter� 3 params
	// n�o haver� vetor de controle
...
void Kalman::estimar(KalmanFilter *KF, KalmanState *state, const HoughDoMeio &measurement, bool estimarLargura) {
	...
	// limpar
	if (state->limpar) {
		state->hough = NULL; // n�o h� estimado
		resetaKalman(KF, 6, 3);
	...
	if (state->estaDesativado || state->manter) {
		// n�o atualiza estimado (hough do estado)
		state->manter = false;
...
void Kalman::realizarEstimativa(KalmanFilter *KF, KalmanState *state, const Mat1d &kalmanMeasurement, bool estimarLargura) {
	...
	// altera a matrix de transi��o para que a largura seja ou n�o estimada conforme necess�rio
...
void Kalman::alteraMatrixTransicao(KalmanFilter *KF, bool estimarLargura) {
	if (estimarLargura) {
        ...
        KF->transitionMatrix = (Mat_<double>(KF->transitionMatrix.cols, KF->transitionMatrix.rows) <<
        			...
        			0, 0, 0, 1, 0, 0,		// delta_base
        			0, 0, 0, 0, 1, 0,		// delta_topo
					0, 0, 0, 0, 0, 0); // delta_largura  <-- não estima/atualiza		// delta_largura-- n�o estima/atualiza
	}
}
