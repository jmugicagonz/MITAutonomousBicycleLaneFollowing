...
public:	
	...
	std::vector<cv::Point2d> applyHomographyInv(const std::vector<cv::Point2d> &points);
	void applyHomography( const cv::Mat& _origBGR, cv::Mat& _ipmBGR, int interpolacao = cv::INTER_LINEAR, int borderMode = cv::BORDER_CONSTANT);
	void applyHomographyInv(const cv::Mat& _ipmBGR, cv::Mat& _origBGR, int interpolacao = cv::INTER_LINEAR, int borderMode = cv::BORDER_CONSTANT);
