...
void IPM::applyHomography(const Mat& _inputImg, Mat& _dstImg, int interpolacao, int _borderMode)
{
	// Generate IPM image from src
	remap(_inputImg, _dstImg, m_mapX, m_mapY, interpolacao, _borderMode);//, BORDER_CONSTANT, Scalar(0,0,0,0));
}
void IPM::applyHomographyInv(const Mat& _inputImg, Mat& _dstImg, int interpolacao, int _borderMode)
{
	// Generate IPM image from src
	remap(_inputImg, _dstImg, m_invMapX, m_invMapY, interpolacao, _borderMode);//, BORDER_CONSTANT, Scalar(0,0,0,0));
}
...
vector<Point2d> IPM::applyHomographyInv(const vector<Point2d> &points) {
	...
	for (Point2d p : points) {
		newPoints.push_back(this->applyHomographyInv(p));
	}
	return newPoints;
}
